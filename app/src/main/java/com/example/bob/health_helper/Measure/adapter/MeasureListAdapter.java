...
public class MeasureListAdapter extends RecyclerView.Adapter {
	...
	private List<MeasureData> mWeightListDataList = new ArrayList<>();
		...
		switch (getItemViewType(i)){
			case TYPE_WEIGHT:
				if (mWeightListDataList.size() > 0){
					((MostRecentWeightHolder)viewHolder).bind(mWeightListDataList.get(0).getWeight());
				}
				break;
			case TYPE_GRID:
				if (mWeightListDataList.size() > 0){
			...
			case TYPE_DATA:
				if (mWeightListDataList.size() > 1){
					((MeasureRecordsHolder)viewHolder).bind(mWeightListDataList.get(i - 2));
				}
				break;
		}
	}
	...
	public int getItemCount() {
		return mWeightListDataList.size() + 1;
	}
	...
	public void addData(MeasureData bodyData){
		mWeightListDataList.add(bodyData);
